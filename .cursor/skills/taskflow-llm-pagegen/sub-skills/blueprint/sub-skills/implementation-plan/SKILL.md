...
Phase 3 的 `implementation_plan` 是后续 state model、bbox、组件选择和页面生成的主要设计依据，因此必须给出足够完整的 UI 设计说明。禁止只写“展示信息卡片 / 展示列表 / 弹出弹窗 / 进入详情页”这类摘要。

每个候选方案必须覆盖：

- 页面结构：从上到下列出主要区域、容器和固定区域。
- 容器层级：说明哪些是页面容器、卡片、列表、Bottom Sheet、遮罩、底部操作栏。
- 组件内部元素：对卡片、列表项、弹窗、底部操作栏等复合组件，必须说明内部包含哪些字段、图标、按钮、标签和辅助文案。
- 布局关系：说明横向/纵向排列、主次信息、左中右区域、顶部/底部固定区、卡片间距。
- 关键文案：写出标题、按钮、提示、占位符、状态标签、金额、说明文字等可见文案。
- 状态变化：说明继承哪些区域、新增哪些区域、哪些按钮或字段状态发生变化。

复合组件必须展开到可执行粒度：

- 卡片：必须说明卡片标题、主信息、辅助信息、图片/图标、状态标签、操作入口、内部排列。
- 列表：必须说明列表项结构、每项字段顺序、主次信息、右侧操作、分割线或卡片间距。
- Bottom Sheet / Drawer / Modal：必须说明高度比例、遮罩、z-index 层级、顶部标题栏、关闭/清空操作、主体内容、底部固定操作区和主按钮。
- 底部操作栏：必须说明左/中/右区域划分、图标入口、主次按钮、按钮文案和大致宽度关系。
- 表单：必须说明字段 label、占位符、默认值、错误/禁用/loading 状态和确认按钮位置。

### 高度扩展策略

当页面、卡片列表或详情内容较多时，采用“扩展 state/page 高度”的策略，而不是压缩卡片内容、减少字段、做空卡片或让容器内部滚动。

- 如果内容超过初始视口高度，明确说明该 state 的画布高度可以向下扩展，后续 state model 应生成超过 `initial_height` 的 bbox 和 state `height`。
- 不要把多个信息块压缩成入口宫格来节省高度，除非 brief 明确要求入口宫格。
- 不要为了塞进一屏而省略卡片内部字段；优先保持信息完整，再扩展页面高度。
- Bottom Sheet / Modal 这类弹层仍应控制在合理高度；如果主体内容过多，应扩大该弹层所属 state 的整体高度或重新组织内容，而不是让关键内容消失。
...
- `implementation_plan` 建议 180–420 字；复杂页面、卡片列表、Bottom Sheet、详情页可以更长，必须包含组件、形式、布局、内部元素、关键文案和高度策略
