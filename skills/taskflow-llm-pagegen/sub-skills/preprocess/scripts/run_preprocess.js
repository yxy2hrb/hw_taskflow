...
async function main() {
  ...
  if (py.error) {
    throw new Error(`replace_body.py could not start with ${pythonCommand}: ${py.error.message}`);
  }
  if (py.status !== 0) {
    throw new Error(`replace_body.py failed with ${pythonCommand}: ${py.stderr || py.stdout || `exit ${py.status}`}`);
  }
