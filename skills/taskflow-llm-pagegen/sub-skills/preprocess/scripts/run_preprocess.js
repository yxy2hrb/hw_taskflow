...
function resolvePythonCommand() {
  ...
  throw new Error("Python interpreter not found. Install Python 3 and ensure python3 is available in PATH.");
}
...
async function main() {
  ...
  const py = spawnSync(pythonCommand, [
    REPLACE_BODY_SCRIPT, "--target", htmlPath, "--body", annotatedBodySemanticPath, "--out", outputHtmlPath
  ], { encoding: "utf8" });
  if (py.error || py.status !== 0) {
    throw new Error("replace_body.py failed: " + (py.error?.message || py.stderr || py.stdout || `exit ${py.status}`));
  }
