...
allowed-tools: Bash(npx shadcn@latest *), Bash(pnpm dlx shadcn@latest *), Bash(bunx --bun shadcn@latest *), Bash(node skills/scripts/validate_design_system_resources.mjs), Bash(npm run build), Bash(npm run build-storybook), Bash(pnpm run build), Bash(pnpm run build-storybook), Bash(bun run build), Bash(bun run build-storybook)
---
...
2. **Detect resource mode** — if the project exposes `src/route-index.md`, `src/pages-specs/layout/`, `src/blocks-specs/blocks.json`, and `src/components-specs/components.json`, use the resource-aware workflow. Otherwise infer a temporary route/layout plan from the request and existing project files.
3. **Route matching** — for resource-aware projects, read `src/route-index.md` and map the request to `page_type`. Use the route fallback only when the request is ambiguous.
4. **Layout resolution** — read `src/pages-specs/layout/{page_type}.md` and extract `layout_skeleton`, `reference_blocks`, `needed_components`, `composition_mapping`, and `generation_constraints`.
5. **Source grounding** — resolve `reference_blocks` through `src/blocks-specs/blocks.json`, resolve `needed_components` through `src/components-specs/components.json`, resolve related visual assets through `src/assets/assets.json` plus any `related_assets` or `asset_mapping` declared in the matched layout/template, then read the mapped TSX, stories, CSS, tokens, and asset files. Component markdown is on demand only. Never use generated preview output as a grounding source. This includes any content under future `src/render/**`, `render/**`, `Render/**`, or any similarly named render-output folder in the workspace.
...
7. **Validate** — run the available checks, such as `node skills/scripts/validate_design_system_resources.mjs`, `npm run build`, or `npm run build-storybook`. If a check is unavailable, report why.
...
Read [references/page-generation.md](./references/page-generation.md) for the full page generation procedure. Read [references/resource-contract.md](./references/resource-contract.md) for the `src/components-specs` schema, markdown read policy, layout schema, and block/component mapping rules.

### Resource-Aware Rules

- `route-index.md` and `layout/{page_type}.md` are default inputs for page generation.
- `src/assets/assets.json` is the default visual asset index when it exists.
...
- Any `render` / `Render` folder is generated output unless the user explicitly says otherwise. Do not use files from future `src/render/**`, `render/**`, `Render/**`, or other render-output directories as template/reference sources for new page generation.
...
- TSX, stories, CSS, tokens, and assets registered in `src/assets/assets.json` are the true source for generated code.
- `src/blocks-specs/blocks.json` is the page generation index; do not treat it as a shadcn CLI registry unless it is explicitly converted to a valid shadcn registry schema.
...
- [references/resource-contract.md](./references/resource-contract.md) — `src/components-specs` structure, layout markdown schema, block/component mapping, registry boundaries
